...
from albef.modeling.model_vqa import ALBEF_CLS, ALBEF_GEN, ALBEF_NLVR, ALBEF_NLVR2
...
class NLVRDataset(Dataset):
    # NLVR dataset
    def __init__(self, caption_file, img_transform, image_dir, tokenizer, args, is_train=True):
        """
        ...
        self.data = []
        logger.info('loading dataset from {} with images from {}'.format(caption_file, image_dir))
        with open(caption_file, 'r') as rf:
            for line in rf:
                info = json.loads(line)
                self.data.append([info['id'], info['caption'], info['label'], info['left_img'], info['right_img']])
    ...
    def __getitem__(self, index):
        question_id, question, answer, left_image_id, right_image_id = self.data[index]
        left_feature = self.get_image(left_image_id)
        right_feature = self.get_image(right_image_id)
        ...
        return index, (left_feature, right_feature, question, answer_id)
...
def train(args, train_dataset, val_dataset, model, tokenizer):
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, (_, batch) in enumerate(train_dataloader):
            model.train()
            l_images, r_images, text, answer = batch
            if global_step == 0:
                print(text, answer)
            ...
            l_images = l_images.to(args.device)
            r_images = r_images.to(args.device)
            bs = l_images.shape[0]
            # print('before', text.input_ids.shape)
            outputs = model(l_images, r_images, text.input_ids, text.attention_mask, answer)
            loss, logits = outputs
            if args.n_gpu > 1: 
        ...
        if args.evaluate_during_training:
            ...
            if full_scores is not None:
                epoch_log['full_scores'] = full_scores
...
def main():
    ...
    if albef_config['model_type'] == 'nlvr': 
        model_class = ALBEF_NLVR
    elif albef_config['model_type'] == 'nlvr2':
        # simplified version
        model_class = ALBEF_NLVR2
    else:
        raise NotImplementedError
    ...
    if args.do_train:
        train_dataset = NLVRDataset(caption_file=args.data_file_train, image_dir=train_image_dir, img_transform=train_transform, tokenizer=tokenizer, args=args, is_train=True)
        if args.evaluate_during_training:
            if isinstance(args.data_file_val, dict):
                val_dataset = {k:NLVRDataset(caption_file=v, image_dir=val_image_dir[k], img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False) for k,v in args.data_file_val.items()}
            else:
                val_dataset = NLVRDataset(caption_file=args.data_file_val, image_dir=val_image_dir, img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False)
        else:
            val_dataset = None
    ...
    if args.do_test or args.do_eval:
        if args.eval_all_checkpoints:
            ...
            test_dataset = NLVRDataset(caption_file=args.data_file_val, img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False)
            ...
            # args = restore_training_settings(args)
            if isinstance(args.data_file_val, dict):
                test_dataset = {k:NLVRDataset(caption_file=v, image_dir=val_image_dir[k], img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False) for k,v in args.data_file_val.items()}
            else:
                test_dataset = NLVRDataset(caption_file=args.data_file_val, image_dir=val_image_dir, img_transform=val_transform, tokenizer=tokenizer, args=args, is_train=False)
            checkpoint = args.eval_model_dir
            assert op.isdir(checkpoint)
            logger.info("Evaluate the following checkpoint: %s", checkpoint)
