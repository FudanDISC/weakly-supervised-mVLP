...
def fetch_img_func(q, img_download_root, total_len):
    ...
    while True:
        ...
        if isinstance(file_id, int):
            img_save_path = os.path.join(img_download_root, '{:07}.jpg'.format(file_id))
        else:
            img_save_path = os.path.join(img_download_root, '{}.jpg'.format(file_id))
...
def main():
    ...
    parser.add_argument('--json_file', help='dir path for csv file', required=True)
    ...
    parser.add_argument('--url_index', type=str, default=None, help='the column index for url in the tsv file')
    ...
    if not os.path.exists(json_file):
        logger.error('csv dir %s not exists', json_file)
    ...
    with open(json_file, 'r') as rf:
        json_lines = json.load(rf)
        if args.url_key is not None:
            json_lines = json_lines[args.url_key]
        for line_id in tqdm.tqdm(range(start_id, end_id), desc='loading the data'):
            line = json_lines[line_id]
            if args.url_index is not None:
                line = line[args.url_index]
            img_id = line.split('/')[-1].split('.')[0]
            download_info = (img_id, line_id, line)
            if os.path.exists(os.path.join(img_dir, '{}.jpg'.format(img_id))):
                continue
            q.put(download_info)
                # if img_id > 10000:
                #     break
